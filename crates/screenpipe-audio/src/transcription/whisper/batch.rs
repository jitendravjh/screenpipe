...
/// Detects the dominant language of an audio buffer with whisper, honoring the
/// `languages` allow-list (a single entry is forced, empty means detect any).
/// Returns `None` when the audio is too quiet to judge. The local live path uses
/// this to lock one language for the session instead of re-detecting every short
/// chunk, which otherwise flips scripts mid-meeting (issue #5650).
